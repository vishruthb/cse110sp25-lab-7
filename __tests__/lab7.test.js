describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
   ...
   for (let i = 0; i < prodItemsData.length; i++) {
    ...
    if (title.length === 0 || price.length === 0 || image.length === 0) {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const rootE = await shadowRoot.asElement();
    const button = await rootE.$('button');
    await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const item of items) {
      const root = (await item.getProperty('shadowRoot')).asElement();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartCnt = await page.$eval('#cart-count', el => el.innerText);
